...
import cl.cc5604.fcbarcelonaonline.business.application.captcha.CageCaptchaServlet;
...
//        logger.info("attemptAuthentication, captchaCode:"+captchaCode+", captchaResponse:"+captchaResponse);

        // perform validation of captcha

        if (captchaCode == null || captchaResponse == null) {
//            logger.info("Invalid session, some or all attributes are null");
            errorMsg = "Invalid session.";
        } else if (!captchaCode.equals(captchaResponse)) {
//            logger.info("Invalid captcha, captchaCode:"+captchaCode+", captchaResponse:"+captchaResponse);
